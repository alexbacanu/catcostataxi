...
export const fetchLegal = cache(async (type: string, lang: string) => {
  ...
  const allLegal = await legal.find({ type, lang }).project<Legal>({ _id: 0 }).sort({ version: -1 }).toArray();

  if (allLegal.length == 0) console.warn(`😱 Warning: No legal documents found with this type: ${type}`);
...
export const fetchSingleLegal = cache(async (type: string, lang: string, version?: string) => {
  ...
  if (!singleLegal)
    console.warn(`😱 Warning: No legal documents found with this type: ${type} and version: ${version}`);
